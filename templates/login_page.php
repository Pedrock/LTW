		...
		<div id="content" class="login" >
			...
			<div id="register" class="box">
				<form action="register" method="POST">
					...
					<div id="error-reg-fname" class="error-reg" <?php if (!empty($_GLOBALS['LOGIN']['fname'])) echo 'style="display:block"' ?>>
						<?php echo $lang['FIRST_NAME_ERROR'] ?>
					</div>
				
					<input type="text" id="lname" name="lname" placeholder="<?php echo $lang['LAST_NAME'] ?>"
					value="<?php if(isSet($_POST['lname'])) echo $_POST['lname'] ?>">
					<div id="error-reg-lname" class= "error-reg" <?php if (!empty($_GLOBALS['LOGIN']['lname'])) echo 'style="display:block"' ?>>
						<?php echo $lang['LAST_NAME_ERROR'] ?>
					</div>
					
					<input type="text" name="email" placeholder="<?php echo $lang['EMAIL'] ?>"
					value="<?php if(isSet($_POST['email'])) echo $_POST['email'] ?>">
					<div id="error-reg-email" class= "error-reg" <?php if (!empty($_GLOBALS['LOGIN']['email'])) echo 'style="display:block"' ?>>
						<?php echo $lang['EMAIL_ERROR'] ?>
					</div>
					<div id="reg-email-in-use" class= "error-reg" <?php if (!empty($_GLOBALS['LOGIN']['email-in-use'])) echo 'style="display:block"' ?>>
						<?php echo $lang['EMAIL_IN_USE'] ?>
					</div>

					<input type="password" name="password" placeholder="<?php echo $lang['PASSWORD'] ?>">
					<div id="error-reg-pass" class= "error-reg" <?php if (!empty($_GLOBALS['LOGIN']['password'])) echo 'style="display:block"' ?>>
						<?php echo $lang['PASSWORD_LENGTH'] ?>
					</div>
					
					<input type="password" name="password2" placeholder="<?php echo $lang['CONFIRM_PASSWORD'] ?>">
					<div id="error-reg-pass2" class= "error-reg" <?php if (!empty($_GLOBALS['LOGIN']['password2'])) echo 'style="display:block"' ?>>
